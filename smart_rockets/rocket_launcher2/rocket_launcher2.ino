...
#define INDUCTION_POWER_PIN		(4)
#define INDUCTION_SELECT_1_PIN	(5)
#define INDUCTION_SELECT_2_PIN	(6)

#define COMMAND_BUFFER_SIZE		(100)
#define COMMAND_TIMEOUT_MS		(150)
#define SCREEN_SAVER_S			(40)
#define INDUCTION_RELAY_MS		(70)
...
void set_induction_coils(uint8_t mask) {
	digitalWrite(INDUCTION_SELECT_1_PIN, mask & 0x01);
	digitalWrite(INDUCTION_SELECT_2_PIN, mask & 0x02);
}

void set_induction_power(bool is_on) {
	digitalWrite(INDUCTION_POWER_PIN, !is_on);
}

void get_induction_power() {
	return !digitalRead(INDUCTION_POWER_PIN);
}

void set_induction_coil(uint8_t channel) {
	if (get_induction_power()) {
		if (channel) {
			set_induction_power(false);
			delay(INDUCTION_RELAY_MS);
			set_induction_coils(0);
		}
	}
	else if (channel){
