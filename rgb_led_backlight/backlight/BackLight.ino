...
uint8_t ledBuff[LED_BUFF_LEN]; // volatile
...
void msgSendStream()
{
	...
	memcpy(retBuff + MSG_LEDS, ledBuff, LED_BUFF_LEN);
	msgSend();
}
...
{
	mainMode &= ~MAIN_MODE_RUN_EFFECT;
	memcpy(ledBuff, msgBuff + MSG_LEDS, LED_BUFF_LEN);
	updateLEDs();
}
...
void effectColorFromLeds(uint16_t leds)
{
	...
	for (uint8_t i = 0; i < LED_COUNT; i++, mask <<= 1)
		if (leds & mask)
		{
			memcpy(effectColor, ledBuff + i * RGB_COUNT, RGB_COUNT);
			return;
		}
}
...
{
	memcpy(ledBuff + index * RGB_COUNT, effectColor, RGB_COUNT);
}
...
void fillLedsBlack()
{
	memset(ledBuff, 0, LED_BUFF_LEN);
}

void fillLedsWhite()
{
	memset(ledBuff, COLOR_MAX, LED_BUFF_LEN);
}

void fillLeds(const uint8_t* rgbBuff)
{
	for (uint8_t i = 0; i < LED_BUFF_LEN; i += RGB_COUNT)
		memcpy(ledBuff + i, rgbBuff, RGB_COUNT);
}

void fillLedBlack(uint8_t index)
{
	memset(ledBuff + index * RGB_COUNT, 0, RGB_COUNT);
}
...
void updateLEDs()
{
	if (mainMode & MAIN_MODE_POWER) sendArrToLEDs(ledBuff);
}

void sendArrToLEDs(uint8_t *data)
{
	cli(); // Disable global interrupts
	uint8_t curByte, ctr, maskLo, maskHi = _BV(DATA_PIN), curLed = LED_COUNT;
